...
from libs.golib.core.goterm import GOTerm
...
class SemanticSimilarityMixin(metaclass=abc.ABCMeta):
    ...
    def __subclasshook__(cls, subclass):
        return (
            hasattr(subclass, "gene_wise")
            and callable(subclass.gene_wise)
            and hasattr(subclass, "term_wise")
            and callable(subclass.term_wise)
            or NotImplemented
        )

    @abc.abstractmethod
    def term_wise(
        self, ontology: GeneOntology, organism_name: str
    ...
    def lowest_common_ancestor(
        self,
        ancestors_a: Set[GOTerm],
        ancestors_b: Set[GOTerm],
        organism_name: str,
        domain: str,
    ) -> float:
        domain_count = self.ontology._annotation_counts_cache[organism_name][
            domain
        ]  # noqa
...
class Resnik(SemanticSimilarityMixin):
    ...
    def _build_ancestor_cache(self, organism_name: str) -> None:
        ...
        for go_id, term in track(
            self.ontology._terms.items(), description="Building ancetry cache"
        ):
            if len(term.annotations[organism_name]) > 0:
                self._ancestor_cache[organism_name][term] = term.ancestors()

    def term_wise(self, organism_name: str) -> Dict[str, npt.NDArray]:  # type: ignore # noqa
        self._build_ancestor_cache(organism_name)
        ss = {}
        for domain in GeneOntology.DOMAINS:
            terms = [
                t
                for t in self._ancestor_cache[organism_name].keys()
                if t.domain == domain
            ]
            N = len(terms)
            ss[domain] = {
            ...
            for i in track(
                range(N),
                description=f"Calculating {domain} semantic similarity...",  # noqa
            ):
                for j in range(i, N):
                    r = -np.log(
    ...
    def gene_wise(  # type: ignore
        self, ontology: GeneOntology, organism_name: str, domain: str
    ) -> Dict[str, npt.NDArray]:
